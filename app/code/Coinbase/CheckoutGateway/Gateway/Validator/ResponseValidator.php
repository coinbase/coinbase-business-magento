...
class ResponseValidator extends AbstractValidator
{
    ...
    /**
     * Validate the checkout API response.
     *
    ...
    public function validate(array $validationSubject): ResultInterface
    {
        ...
        if ($httpStatus < 200 || $httpStatus >= 300) {
            ...
            $errorMessage = $response['errorMessage'] ?? 'Checkout creation failed';
        ...
        if (empty($response['id'])) {
            $isValid = false;
            $errorMessages[] = __('Checkout response missing ID.');
        }

        if (empty($response['url'])) {
            $isValid = false;
            $errorMessages[] = __('Checkout response missing URL.');
        }
        ...
        if (!isset($response['status']) || $response['status'] !== 'ACTIVE') {
            $isValid = false;
            $errorMessages[] = __(
                'Checkout status is "%1", expected "ACTIVE".',
